...
The default URL format is `https://api.fireflylx.com/ota/$$class$$/$$product_hex$$/$$class$$?current_version=$$current_version$$`.  The third path segment (`$$class$$`) is the application name, which equals the hardware class for both Controllers and Clients.
