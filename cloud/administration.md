...
Each user is granted access to one or more environments at invite time.  Environment access is stored in DynamoDB and checked at both invite time and sign-in.  A user with `dev` access can only sign in to the `dev` environment; a user with `production` access can only sign in to the `production` environment.  Access to both environments can be granted simultaneously.
