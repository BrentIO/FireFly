...
| [dynamodb-devices](./dynamodb-devices.md) | Table must exist before the function is deployed and granted read/write access |
| [shared-layer](./shared-layer.md) | Lambda layer must exist before function deployment |
| [s3-device-backups](./s3-device-backups.md) | S3 bucket must exist before the function is deployed and granted put/head access |
...
- `s3:PutObject`, `s3:HeadObject` on the device backups bucket
...
5. Install Python arm64 dependencies into `lambdas/func-api-devices-backup-post/`.
6. SAM deploy `firefly-func-api-devices-backup-post` with parameters:
   - `ApiId`
   - `SharedLayerArn`
...
1. Configure AWS credentials.
2. SAM delete `firefly-func-api-devices-backup-post`.
3. Delete CloudWatch log group `/aws/lambda/firefly-func-api-devices-backup-post`.
